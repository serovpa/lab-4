...
большое количество параметров, что делает использоавние конструктора
неудобным.
...
времени моменты инициализации различных атребутов сложного объекта, либо
...
котрое может быть достаточно сложным:
```java
/**
...
```
У предложенного интерфейса может быть предусмотренно множество 
реализаций, которые мы можем хотель инкапсулировать на уровне модуля.
Но, при этом, мы хотим предоставить возможность создавать экземпляры
сервиса в контекстах, в котроых они могут быть полезны.

Для этих случаев и придуман эталон проектирования "Фабрика". 

В самой простой реалзации мы можем создать статический метод, который 
возврящает реализацию обобщая её до описанного интерфейса.
```java
/**
  * Сервис приложения, отвечающий за аутентификацию и авторизацию 
  * пользователей системы.
...
Примером может полужить конфигурация приложения. Допустим конфигурация
...
```
Мы можем хотель в различных ситуациях хранить конфигурацию в различных 
файлах. Наприме, в виде текстовых файлах, или на специальном сервере, или
в реестре операионной системы. В таком случае мы можем создать несколько
фабричных методов, с одинаковым названием и различными параметрами. Каждый
метод будет возврящать разные реализации, имеющие общий нитерфейс.
```java
/**
  * Конфигурация приложения.
  */
public interface Configuration {
    ...
    /**
      * Возвращает концигурацию, ханимую в файле.
      * 
    ...
    /**
      * Возвращает концигурацию, ханимую на сервере конфигураций.
      * 
